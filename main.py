...
from modules.logging import log_error, log_info, log_warning
...
vec_context="this is a test context for vector store"
...
class RealtimeAPI:
    ...
    async def send_initial_prompts(self, websocket):
        contextvec = "Please respond in English: " + vec_context
        logger.info(f"Sending {len(self.prompts)} prompts: {self.prompts}")
        content = [{"type": "input_text", "text": contextvec}]
        event = {
